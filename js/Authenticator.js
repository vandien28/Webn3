const container1 = document.querySelector(".container1"),
    ...
    checkusername.addEventListener('input',function(event){
        ...
        if(valueusername == "") {
            ...
            checkcircle.classList.add("hide")
            return
        } if(valueusername != createaccount.map(item => item.username)){
...
formlogin.addEventListener('submit', check => {
    ...
    let checkLogin = listAccount.some(value => value.name === nameLogin && value.password === passwordLogin)
    var createaccount = localStorage.getItem("create") ? JSON.parse(localStorage.getItem("create")) : [];
    var userCLogin  =  createaccount.map(value => value.username === nameLogin && value.password === passwordLogin)
    if(checkLogin == true) {
            window.location.href = "../Html/Admin.html"
            return
    }  if (userCLogin[0] == true) {
        ...
        name1.addEventListener('input',function(check) {
            let checkemail = check.target.value
            wrong.classList.add('hide')
        })
        password1.addEventListener('input',function(check) {
            let checkpassword = check.target.value
            wrong.classList.add('hide')
        })
    }
})
