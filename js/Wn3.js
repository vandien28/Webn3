...
const listAccount = [
    {
        email: "dien2003@gmail.com",
        password: "1234567890"
    },
    {
        email: "chuong2003@gmail.com",
        password: "1234567890"
    },
    {
...
function clickinfo() {
    ...
    edit = document.querySelector(".management-edit").style.display = "none"
...
function removeimg() {
    clearimg = document.querySelector(".img").src = ' '
}
...
function editimg() {
    ...
    const inputs = document.getElementById("file-inputs");
    inputs.addEventListener("change", (i) => {
        if (i.target.files.length) {
            const srcs = URL.createObjectURL(i.target.files[0]);
            img.src = srcs
        }
    });
}
